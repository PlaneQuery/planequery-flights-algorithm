...
from flights_ai_model_airport.features import TrainingDataCache, build_training_data
...
def train(
    training_dates,
    validation_date=date(2026, 3, 15),
    max_training_flights: int | None = None,
    training_data_cache: TrainingDataCache | None = None,
):
    ...
    for endpoint in AIRPORT_MODEL_ENDPOINTS:
        X, y, groups, airport_idents = build_training_data(
            training_dates,
            endpoint=endpoint,
            max_flights=max_training_flights,
            cache=training_data_cache,
        )
    ...
    import joblib
    path = get_model_path(
        num_days=len(training_dates),
        num_flights=max_training_flights,
    ) / "model.pkl"
